...
- **Simplicity and Interpretability**: Easy to understand and explain to non-statistical folks.
...
**Weaknesses**:
-**rely on metrics**: over relying on all -in-one metrics possible miss significant stats 
...
- **metrics improvement** : I want to spend more time from scratch to do a EDAfrom scratch to discover more factors that contribute to game success instead of relying on all-in-one metrics.
...
selected_teams <- c("BOS", "BKN", "OKC", "DAL","IND","NYK")

data_reg_compare_teams <- data_reg_compare %>% filter(season == 2023 & off_team %in% selected_teams)
df_long <- data_reg_compare_teams %>%
  select(off_team, ORTG, ORTG_PT) %>%
  pivot_longer(cols = c(ORTG, ORTG_PT), names_to = "period", values_to = "rating")


ggplot(df_long, aes(x = off_team, y = rating, fill = period)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "ORTG vs ORTG_PT for Selected Teams", x = "Team", y = "Offensive Rating (ORTG)") +
  theme_minimal() +
  scale_fill_manual(values = c("ORTG" = "red", "ORTG_PT" = "blue"), labels = c("Entire Season", "Post Trade Deadline"))
### testing 
...
prediction_model <- train(
  ...
  data = playoff_data_with_player, 
  method = "glm", 
  family = "binomial", https://freemanbrain.space/
  trControl = train_control
)
# Summarize the model,Extract predictions
